...
Canonical public paths use `/frametales/`. Legacy `/cameramark/` pages redirect to
the corresponding new pages for compatibility. The September 23, 2026 migration
changes URL paths only; it does not change privacy or purchase provisions.
...
- Destination directory: `frametales/`
- Expected public root: `https://zzsoftservice.github.io/frametales/`
...
1. Recheck the site repository status and confirm `frametales/` is still absent or
   inspect its current contents before changing it.
2. Copy this directory to the site repository as `frametales/`. Do not delete or
   overwrite any sibling directory.
3. Keep `cameramark/` compatibility redirects and inspect both scoped directory diffs.
4. Stage only `frametales/` and `cameramark/`; verify the staged name-status
